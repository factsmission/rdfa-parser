...
function parseElement(element, prefixMappings, defaultPrefixMapping, noPrefixMapping, subject, target) {
    ...
    if (element.nodeType === 1) {
        ...
        if (element.getAttribute("typeof") === "rdfa:Pattern") {
            ...
            subject = element.getAttribute("resource") ? evaluateRealtiveURI(element.getAttribute("resource")) : element.getAttribute("typeof") ? _dataModel2.default.blankNode() : subject;
            if (element.getAttribute("typeof")) {
                ...
                element.getAttribute("property").trim().split(/\s+/).forEach(function (property) {
                    ...
                    if (property === "rdfa:copy") {
                        ...
                        var object = element.getAttribute("content") ? _dataModel2.default.literal(element.getAttribute("content"), dataType) : element.href ? _dataModel2.default.namedNode(new URL(element.href, window.location.href).href) : element.src ? _dataModel2.default.namedNode(new URL(element.src, window.location.href).href) : _dataModel2.default.literal(element.innerHTML, dataType);
                        invokeTarget(subject, predicate, object);
                    }
                });
            }
...
function evaluateRealtiveURI(relativeURI) {
    return _dataModel2.default.namedNode(new URL(relativeURI, window.location.href).href);
}
...
function parse(element, target, base, useInitialContext) {
    var currentSubject = _dataModel2.default.namedNode(base || window.location);
